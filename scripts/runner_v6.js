...
const PART_FILES_MAP = {
  ...
  NIST_Part1: {
    ...
    strategy_with_rationale: "NIST_Part1_rationale.json",
  },
  NIST_Part2: {
    features: "features_NIST_2.json",
    part_context: "part_context_NIST_2.json",
    machine_context: "machine_context_NIST_1.json",
    strategy_with_rationale: "NIST_Part2_rationale.json",
  },
};
...
const CONFIG = {
  ...
  STRATEGY_WITH_RATIONALE_PATH: path.join(
    BASE_DIR,
    `outputs/rationale/${PART_FILES_MAP[PART].strategy_with_rationale}`,
  ),
  ...
  PROMPT_PATHS: {
    PART_STRATEGY: path.join(
      BASE_DIR,
      "prompts/v6/part_level_strategy.prompt.txt",
    ),
    FEATURE_REFINEMENT: path.join(
      BASE_DIR,
      "prompts/v6/feature_strategy_refinement.prompt.txt",
    ),
  },
...
/* =========================
   PROMPT BUILDERS
========================= */
function buildStrategyRefinementPrompt(feature, baselineStrategy, rationale) {
...
function buildPartStrategyPrompt(features, featureStrategies) {
...
async function refineStrategies(features, originalStrategies) {
  ...
  for (const [index, key] of strategyKeys.entries()) {
    logger.log(
      `In process: Refining strategy for feature ${index + 1}/${strategyKeys.length}`,
    );
    const strategy = { passes: originalStrategies[key].passes ?? [] };
    const rationale = originalStrategies[key].rationale ?? "";
    ...
    const prompt = buildStrategyRefinementPrompt(feature, strategy, rationale);
...
async function generatePartStrategy(features, featureStrategies) {
  const prompt = buildPartStrategyPrompt(features, featureStrategies);
...
async function run() {
  ...
  const originalFeatureStrategies = readJson(
    CONFIG.STRATEGY_WITH_RATIONALE_PATH,
  );
